...
int main(void){
	MuInit();
	MuMaterial acordesA, acordesB, afinal, auxiliar, improvisoT, improvisoP, improvisoA, improvisoB;
	...
	int inf_acordeA[8];
	int inf_acordeB[8];
	float celulas[11][4]={{0.5, 0, 0, 0},{ 0.25,0.25, 0,0 },{ 0.375, 0.125, 0, 0},{0.125, 0.375, 0, 0},{ 0.165, 0.170, 0.165, },{0.335, 0.165, 0, 0},{0.165, 0.335, 0, 0},{0.125,0.125 ,0.125 ,0.125 },{0.250, 0.125, 0.125, 0},{0.125, 0.125, 0.250, 0},{0.125, 0.250, 0.125, 0}};


	int i, num_comp, probPitch[10], probPitch2[10];// num. de compassos e probabilidades 
	...
	for(int i=0;i<2;i++)
		afinal+=acordesA;
	afinal.SetInstrument(0,1); //******************** ALTERAR ->>> INSTRUMENTO PIANO HARMONIA
...
/*---------------------------------------------------------------------*/
	...
	for(i=0;i<8;i++){ 
		...
		for(int j=0;j<contador;j++){ //for para gerar cada nota em cada 0,5s
			...
			Preenche_probPitch(inf_acordeA[i],probPitch);
			if(ant!=-1)
				melhora_pitch_prob(probPitch, ant, melodiaA); //melhora probPitch baseado na nota anterior (saltos)
			for(int g =0;g<10;g++)
				probPitch2[g] = probPitch[g]; //pitch2 é o pitc melhorado
			if(ant!=-1)
				nota.SetPitch(SorteiaPitch(melodiaA, probPitch2));
			else
	...
	for(i=0;i<8;i++){ 
		...
		for(int j=0;j<contador;j++){ //for para gerar cada nota em cada 0,5s
			...
			auxiliar = mat_acorde(afinal, compassos[i]);
			Zera_probPitch(probPitch);
			Preenche_probPitch(inf_acordeB[i],probPitch);
			if(ant!=-1)
				melhora_pitch_prob(probPitch, ant, melodiaB); //melhora probPitch baseado na nota anterior (saltos)
			for(int g =0;g<10;g++)
				probPitch2[g] = probPitch[g]; //pitch2 é o pitc melhorado
			if(ant!=-1)
				nota.SetPitch(SorteiaPitch(melodiaB, probPitch2));
			else
				nota.SetPitch(estrutural(melodiaB,auxiliar,-1));
			improvisoB += nota;
			ant = nota.Pitch();
		}
		}
	...
	for(int i=0;	improvisoB.GetNote(0,i).Start()<4;i++)
		improvisoT+=improvisoB.GetNote(0,i);
	...
	for(int i=0;	improvisoB.GetNote(0,i).Start()<12;i++){
		if(improvisoB.GetNote(0,i).Start()>=8)
			improvisoT+=improvisoB.GetNote(0,i);
	}
	...
	for(int i=0;	improvisoB.GetNote(0,i).Start()<8;i++){
		if(improvisoB.GetNote(0,i).Start()>=4)
			improvisoP+=improvisoB.GetNote(0,i);
	} 
	nota.SetStart(88);
	nota.SetAmp(0);
	nota.SetDur(4);
	nota.SetPitch(60);
	improvisoP += nota;
	for(int i=0;	(improvisoB.GetNote(0,i).Start()+improvisoB.GetNote(0,i).Dur())<=16;i++){
		if(improvisoB.GetNote(0,i).Start()>=12)
			improvisoP+=improvisoB.GetNote(0,i);
		if((improvisoB.GetNote(0,i).Start()+improvisoB.GetNote(0,i).Dur())==16)
			break;
	}
	...
	afinal.SetVoice(1, improvisoT, 0);
	afinal.SetVoice(2, improvisoP, 0);
	afinal.SetInstrument(2,3);  //******************** ALTERAR ->>> INSTRUMENTO PIANO MELODIA
	afinal.SetInstrument(1,4); //******************** ALTERAR ->>> INSTRUMENTO TROMPETE MELODIA
