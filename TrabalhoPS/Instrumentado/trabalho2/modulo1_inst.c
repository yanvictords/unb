...
int INST_file_size(char* file_name){
...
void INST_corrigeNome(char nome[100]){
	char auxiliar[100];
	int flag = 0, i=0;
...
void INST_corrigeNome2(char nome[100]){
	char auxiliar[100];
	int flag = 0, i = 0;
...
listaDeAlunos* INST_txtDiscentesEmLista(char* arquivo_doutorado, char* arquivo_mestrado){
	...
	if (doutorado == NULL){
		...
		if (file_size(arquivo_doutorado)==0){
			printf("Arquivo %s vazio!", arquivo_doutorado);
			return Lista;
		}
	}
	...
	if (mestrado == NULL){
		...
		if (file_size(arquivo_mestrado)==0){
			printf("Arquivo %s vazio!", arquivo_mestrado);
			return Lista;
		}
	}
...
listaDeProfessores* INST_txtDocentesEmLista(char* arquivo){
	...
	if (projetos == NULL){
		...
		if (file_size(arquivo)==0){
			printf("Arquivo %s vazio!", arquivo);
			return Lista;
		}
	}	
...
listaDeProjetos* INST_txtProjetosEmLista(char* arquivo){
	FILE* projetos = fopen(arquivo, "r+");
...
listaDeEventos* INST_txtEventosEmLista(char *str, char* arquivo, char* nomepasta){
	...
	char suporte[100]="trabalho2/Arquivos/";
	
/* O codigo a seguir verifica se o diretorio esta com '/' no final antes de adicionar 'trabalho2/Arquivos/'*/
	int k, l;
	k=0;
	while(str[k]!='\0')k++;
	k--;
...
/* O codigo acima verifica se o diretorio esta com '/' no final antes de adicionar 'trabalho2/Arquivos/'*/
...
listaDePeriodicos* INST_txtPeriodicosEmLista(char *src, char* arquivo, char* nomepasta){
	DIR* diretorioDesktop;
    struct dirent* item_desktop;
