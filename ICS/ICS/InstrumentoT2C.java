...
public class InstrumentoT2C extends Dispositivo
{
    ...
    private Alea      geradorRUIDO;   //--unidade de Ruído
    private Oscilador  oscSENOIDAL;    //--unidade Senoidal
    private Oscilador oscF3;
    private Envoltoria envSOMADOR;
    private Envoltoria envRUIDO;
    private Somador somadorAD2;
    private Somador somadorAD3;
    private float fatorCorte;

   public InstrumentoT2C(float fc)
    { this(new Envoltoria(), new Envoltoria(), new Envoltoria(), fc);
	
    }

    public InstrumentoT2C( Envoltoria env_,Envoltoria env_s, Envoltoria env_r, float fc_)
    {  
        super();      
        fatorCorte = fc_;
        this.geradorRUIDO    = new Alea();
        this.envAMPLITUDE    = env_;
        this.envSOMADOR      = env_s;
        this.envRUIDO        = env_r;
        this.oscSENOIDAL = new Oscilador();
	this.oscF3 = new Oscilador();
        this.setRelogio(0);
	...
	oscF3.setDispositivoAmplitude(envAMPLITUDE);
	oscF3.setFrequencia(frequencia);
        oscF3.setFase(fase);
        oscF3.setDuracao(duracao);	              
	oscF3.reset();
	somar();
//	System.out.println(somador.getSaida());
        oscSENOIDAL.setDispositivoAmplitude(envAMPLITUDE);
	oscSENOIDAL.setDispositivoFrequencia(somadorAD3);
    ...
    public void somar()
    { somadorAD2 = new Somador( envSOMADOR,geradorRUIDO );
	somadorAD3 = new Somador(somadorAD2, oscF3);
    }
}
