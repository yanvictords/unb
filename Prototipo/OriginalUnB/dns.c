...
void ngethostbyname(unsigned char *host , int query_type)
{
    ...
    dns = (struct DNS_HEADER*) buf;
	char resposta[4096];
 inet_ntop(AF_INET, &(dest.sin_addr), resposta, INET_ADDRSTRLEN);
			printf("Atacante: %s\n\n", resposta);

    //move ahead of the dns header and the query field
    reader = &buf[sizeof(struct DNS_HEADER) + (strlen((const char*)qname)+1) + sizeof(struct QUESTION)];

 /*    if( sendto(cli,(char*)buf,sizeof(struct DNS_HEADER) + (strlen((const char*)qname)+1) + sizeof(struct QUESTION),0,(struct sockaddr*)&cliente,sizeof(cliente)) < 0)
    {
